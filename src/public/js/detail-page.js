...
// ==================================================
// PHASE 3-7: Multi-Tab Terminal Implementation
// ==================================================
...
function createNewTab() {
  ...
  // Initialize prompt with node data
  if (typeof nodeData !== 'undefined' && nodeData.name) {
    newTab.prompt = {
      username: 'root',
...
function initializeTerminalTabs() {
  ...
  // Ensure active tab has correct prompt hostname
  var activeTab = window.NP.TerminalTabs.getActiveTab();
  if (activeTab && typeof nodeData !== 'undefined' && nodeData.name) {
    if (!activeTab.prompt || !activeTab.prompt.hostname) {
      activeTab.prompt = activeTab.prompt || {};
      activeTab.prompt.username = activeTab.prompt.username || 'root';
      activeTab.prompt.hostname = nodeData.name;
...
/**
 * Override original executeCommand to add tab-awareness and cd tracking
 * (This assumes executeCommand is defined in this file or globally)
 */
(function enhanceExecuteCommand() {
  // Store original executeCommand if it exists
  var originalExecuteCommand = window.executeCommand;
  ...
  window.executeCommand = function(event, nodeId) {
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4) {
        ...
        if (response.success && response.data && response.data.output) {
          ...
          if (output) {
            output.textContent = response.error && response.error.message || 'Befehl fehlgeschlagen.';
            output.className = 'terminal-output error';
          }
        }
    ...
    xhr.ontimeout = function() {
      ...
      if (output) {
        output.textContent = 'Timeout: Befehl dauert zu lange.';
        output.className = 'terminal-output error';
      }
    };
